...
class GWR(GLM):
    ...
    def fit2(self, ini_params=None, tol=1.0e-5, max_iter=20, solve='iwls',
            lite=False, pool=None):
        self.fit_params['ini_params'] = ini_params
        self.fit_params['tol'] = tol
        self.fit_params['max_iter'] = max_iter
        ...
        if solve.lower() == 'iwls':
            ...
            if pool:
                rslt = pool.map(self._local_fit2,
                                range(m))  #parallel using mp.Pool
            else:
                rslt = map(self._local_fit2, range(m))  #sequential
            ...
            if lite:
                ...
                predy = np.array([0]) # predy.flatten() will be called in the constructive function of GWRResults 
                w = None
                if self.hat_matrix:
                    S = None
                else:
